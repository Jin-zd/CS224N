...
\begin{parts}
    ...
    \ifans{
        $2n$. Each word is shifted onto the stack once and popped off the stack once. So, each word is processed twice and the total number of steps is $2n$.
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \ifans{
            \alns{
                \pd{h_i}{x_i} & = \pd{}{x_i} \relu(x_i W_i + b_i) \\
            }
            When $x_i W_i + b_i > 0$, $\pd{h_i}{x_i} = W_i$. Otherwise, $\pd{h_i}{x_i} = 0$.
        } 
        ...
        \ifans{
            \alns{
                \pd{CE(\by, \hat{\by})}{l_i} & = \sum_j \pd{CE(\by, \hat{\by})}{\hat{\by}_{j}} \pd{\hat{\by}_{j}}{l_i} \\
                                            & = \frac{\by}{\hat{\by}_c} \frac{- \exp(-l_i)}{(1 + \exp(-l_i))^2} \\
                                            & = -\frac{\exp(-l_i)}{(1 + \exp(-l_c))(1 + \exp(-l_i))^2}
            }
        }
        ...
        \ifans{
            Best UAS on dev set: 88.77; UAS on test set: 89.72
        } \\
...
(ii) \\
\begin{itemize}
    \item \textbf{Error type}: Coordination Structure Error
    \item \textbf{Incorrect dependency}: elements $\rightarrow$ to
    \item \textbf{Correct dependency}: crucial $\rightarrow$ to
\end{itemize}   
(iii) \\
\begin{itemize}
    \item \textbf{Error type}: Noun Compound Error
    \item \textbf{Incorrect dependency}: decision $\rightarrow$ government
    \item \textbf{Correct dependency}: government $\rightarrow$ decision
\end{itemize}
(iv) \\
\begin{itemize}
    \item \textbf{Error type}: Coordination Structure Error
    \item \textbf{Incorrect dependency}: affects $\rightarrow$ and
    \item \textbf{Correct dependency}: plants $\rightarrow$ and
\end{itemize}
}
...
\ifans{
    Part-of-speech tags can help the parser disambiguate between words that have multiple meanings. For example, the word ``bank'' can be a noun or a verb, and knowing the POS tag can help 
    the parser determine the correct dependency. POS tags can also help the parser understand the grammatical structure of the sentence, which can help it make more accurate predictions.   
}
