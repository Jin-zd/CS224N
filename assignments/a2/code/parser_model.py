...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_wieght = nn.Parameter(torch.empty(self.embed_size * self.n_features, self.hidden_size))
        self.embed_to_hidden_bias = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.xavier_uniform_(self.embed_to_hidden_wieght)
    ...
    def forward(self, w):
        ...
        h = F.relu(torch.matmul(x, self.embed_to_hidden_wieght) + self.embed_to_hidden_bias)
        h = self.dropout(h)
