...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parases = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = partial_parases.copy()
    while len(unfinished_parses) > 0:
        minibatch_parses = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch_parses)
        for (pp, transition) in zip(minibatch_parses, transitions):
            pp.parse([transition])
            if len(pp.buffer) == 0 and len(pp.stack) == 1:
                unfinished_parses.remove(pp)
    dependencies = [pp.dependencies for pp in partial_parases]
    ### END YOUR CODE
