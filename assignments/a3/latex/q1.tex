...
\end{parts}
\ifans{
    The masks are used to prevent the model from attending to the padding tokens in the source sentence. 
    This is important because the padding tokens do not contain any information and should not be used to generate the target sentence.
}
...
\end{lstlisting}
    ...
    \ifans{
        The model's corpus BLEU Score is .
    }
    ...
    \end{subparts}
    \ifans{
        The dot product attention is computationally efficient compared to multiplicative attention, as it requires fewer parameters. 
        However, dot product attention is not able to learn complex relationships between the encoder and decoder hidden states. 
        When the complexity of the relationships is high, dot product attention may have an unstable gradient, which can lead to poor performance.
        The additive attention is able to learn more complex relationships between the encoder and decoder hidden states than multiplicative attention, 
        which makes it more expressive than multiplicative attention.
        When the complexity of the relationships is high, additive attention can maintain a stable gradient, which can lead to better performance.
    }
